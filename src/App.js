...
function App() {
  ...
  const calcConnectPoint = d => {
    ...
    if (
      Math.abs(source.x - target.x) >=
        2.5 * Math.max(source.width, target.width) ||
      Math.abs(source.y - target.y) >=
        5 * Math.max(source.height, target.height)
    ) {
      point.type = "A";
    }
  ...
  const getStrokeWidth = d => {
    const isStart = data.nodes.find(item => item.id === d.__proto__.source)
      .isStart;
  ...
  const handleNodeClick = (d, activity) => {
    activity.select(`.${d.id}`).attr("stroke", "#4285F4");
  ...
  const processAnimation = svg => {
    ...
    const handleEnd = path => {
      ...
      ~index !== 0 && animatedPath.splice(animatedPath[index], 1);
    };
      ...
      csv.forEach((item, csvIndex) => {
        ...
        if (!isEmpty(paths)) {
          ...
          paths.forEach((path, index) => {
            ...
            animateMotion.append("mpath").attr("xlink:href", `#${path.href}`);
            if (index === 0) {
  ...
  const draw = () => {
    d3.forceSimulation(nodes).force(
      "link",
    ...
    svg
      ...
      .append("text")
      .text(
        d => `${Math.round(calcDate(d.startTime, d.endTime) * 100) / 100}hrs`
      )
  ...
  useEffect(() => {
    ...
    currentSvg.current = draw().node();
    temp.append(currentSvg.current);
  ...
  const playAnimation = () => {
    if (!isPause) {
      ...
      currentSvg.current.unpauseAnimations();
    }
  };
